...
export default function FaceDetectionUploader({ onValidationSuccess, onValidationFail }: FaceDetectionUploaderProps) {
  const [status, setStatus] = useState<{ type: 'loading' | 'success' | 'error' | 'idle'; message: string }>({ type: 'idle', message: '' });
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        setStatus({ type: 'loading', message: 'Loading accurate face detection model...' });
        ...
        setStatus({ type: 'idle', message: 'AI ready. Please select a photo.' });
      } catch (error) {
        console.error("Model loading error:", error);
        setStatus({ type: 'error', message: 'Failed to load AI model. Please refresh.' });
      }
    };
    loadModels();
  }, []);
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setStatus({ type: 'loading', message: 'Scanning photo with high accuracy...' });
    ...
    img.onload = async () => {
      try {
        ...
        if (detections.length > 0) {
           ...
           if (bestScore > 0.75) { // Extra strict check against good cartoons
             setStatus({ type: 'success', message: '✅ Valid face detected!' });
             onValidationSuccess(file);
           } else {
             const msg = '⚠️ Cannot clearly verify a real human face. Please try a clearer photo.';
             setStatus({ type: 'error', message: msg });
             onValidationFail?.(msg);
           }
           ...
           const msg = '❌ No human face detected. Please use a real profile photo.';
           setStatus({ type: 'error', message: msg });
           onValidationFail?.(msg);
        }
      } catch (error) {
        console.error("Detection error:", error);
        setStatus({ type: 'error', message: 'Something went wrong during scan.' });
      ...
      <div className="mt-4 space-y-3">
        {status.type === 'loading' && (
          <div className="flex items-center text-blue-500">
            ...
            <span className="text-sm ml-2">{status.message}</span>
          </div>
        )}
        ...
        {previewUrl && (
          <img src={previewUrl} alt="Preview" className="h-32 w-32 object-cover rounded-full border-2 border-gray-300" />
        )}
      </div>
    </div>
  );
